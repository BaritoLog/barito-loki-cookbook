      ...
      block do
        Zip::File.open(cache_file_path) do |zip|
          zip.each do |entry|
            ...
            FileUtils.rm(path) if new_resource.overwrite && ::File.exist?(path) && !::File.directory?(path)
  ...
  if new_resource.overwrite == false && ::File.exist?(new_resource.path)
    ...
    if ::File.directory?(new_resource.source)
      converge_by("zipping #{new_resource.source} to #{new_resource.path}") do
        ...
        new_resource.source << ::File::ALT_SEPARATOR unless new_resource.source =~ /::File::ALT_SEPARATOR$/
